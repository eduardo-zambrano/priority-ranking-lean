/-
  ...
  Violates: TSM, TSI, TM, TI, NCA, GUT, NDTC
-/
...
/-- P-PROT violates Generalized Upward Transfer: x=(2,0) and y=(1,1) have equal
    totals with x₁ < y₁, so GUT demands x ≻ y — but P-PROT strictly prefers
    (1,1), which covers threshold 1 at both states. -/
theorem PPROT_violates_GUT : ¬ Ax_GUT (@PPROT 2) := by
  intro hGUT
  let x : Vec 2 := ![2, 0]
  let y : Vec 2 := ![1, 1]
  have htotal : totalSum x = totalSum y := by
  ...
  exact hstrict.2 (Or.inr pprot_strict_11_20)

/-! ## P-PROT violates NDTC -/

/-- P-PROT violates NDTC: Q-PROT strictly prefers (3,0) to (2,2) (coverage
    advantage at the highest differing threshold 3), so NDTC forbids ranking
    (2,2) strictly above (3,0) — but P-PROT does exactly that (coverage
    advantage at the lowest differing threshold 2). -/
theorem PPROT_violates_NDTC : ¬ Ax_NDTC (@PPROT 2) := by
  intro h
  have hxy : (![3, 0] : Vec 2) ≠ ![2, 2] := by
    intro heq; have := congr_fun heq ⟨0, by omega⟩
    norm_num [Matrix.cons_val_zero] at this
  have hne : (![2, 2] : Vec 2) ≠ ![3, 0] := by
    intro heq; have := congr_fun heq ⟨0, by omega⟩
    norm_num [Matrix.cons_val_zero] at this
  exact not_strict_of_NDTC h hxy qprot_strict_30_22
    (PPROT_strictPart_of_strict hne pprot_strict_22_30)
